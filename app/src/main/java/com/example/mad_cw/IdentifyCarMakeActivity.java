...
//            System.out.println(allDisplayedImages);         // to check whether the arrayList is getting updated
    ...
    public int getRandomCarImage() {
        ...
        Random r = new Random();
//        displayToast(Integer.toString(getHintImage()));         // not needed. generates another random number
        return r.nextInt(11);
    }
    ...
    public void getResult() {         // steps that are followed when the result is required to be shown
        if (mButtonSubmitNext.getText().equals("Submit")) {        // Submit has been clicked
